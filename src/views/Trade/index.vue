<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in userAddress" :key="item.id" @click="changeDefault(item)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="item in tradeList.detailArrayList" :key="item.skuId">
          <li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{consignee.fullAddress}}</span>
        收货人：<span>{{consignee.consignee}}</span>
        <span>{{consignee.phoneNum}}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <a class="subBtn" @click="submitOrders">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState('trade', ['tradeList', 'userAddress']),
    consignee () {
    ...
    async  submitOrders () {
      try {
        const data = {
          ...
          consigneeTel: this.consignee.phoneNum,
          deliveryAddress: this.consignee.fullAddress,
          paymentWay: 'ONLINE',
          orderComment: this.leaveMessage,
          orderDetailList: this.tradeList.detailArrayList
        }
        await this.$store.dispatch('trade/submitOrder', { tradeNo: this.tradeList.tradeNo, data })
