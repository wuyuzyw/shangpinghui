          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item1 in spuSaleAttrList" :key="item1.id">
                ...
                <dd changepirce="0" :class="{active:item.isChecked==1}" v-for="item in item1.spuSaleAttrValueList" :key="item.id" @click="check(item1.spuSaleAttrValueList,item)">{{item.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="num" @change="changeNum">
...
export default {
  ...
  created () {
    this.$store.dispatch('detail/getCartList', this.$route.params.skuId)
  },
  computed: {
    ...mapGetters('detail', ['categoryView', 'price', 'price', 'skuInfo', 'spuSaleAttrList'])
  },
  methods: {
    check (item, target) {
      item.forEach(item => {
    ...
    changeNum () {
      if (typeof (this.num) !== 'number' || this.num < 1) {
        this.num = 1
      }
    },
    async addToCart () {
      try {
        ...
        this.$router.push({ name: 'addcart', query: { skuNum: this.num } })
      } catch (error) {
        alert(error)
      }
    }
